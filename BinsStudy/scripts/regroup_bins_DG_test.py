...
for i, file in enumerate(files): # Only test the last file for now
    for j, channel in enumerate(channels): # Only test the first channel for now
        # create histogram
        histo        = SyntheticData(folder_data, file, channel)
        
        x, y, z      = histo.get_histo()

        enu_regrouped, ct_regrouped, histo_regrouped = histo.regroup_bins_DG(n_pois_norm = 25)
        ...
        print("Check cols: ", histo_regrouped[:, 0])

  

        pcm = ax[i,j].pcolormesh(enu_regrouped,    # x edges: shape (n_enu + 1,)
        ...
        cbar = fig.colorbar(pcm, ax=ax[i,j], pad=0.01)
        cbar.set_label(channel, fontsize=12)

        ax[i,j].set_xscale('log')
        ax[i,j].set_xlabel('Neutrino Energy [GeV]', fontsize=12)
        ax[i,j].set_ylabel(r'$\cos\theta$', fontsize=12)
        ax[i,j].set_title(f"Regrouped — {file}/{channel}\nshape: {histo_regrouped.shape}")
